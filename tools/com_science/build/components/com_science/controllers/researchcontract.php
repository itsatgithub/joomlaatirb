 ...
 * @since 1.5
 */
class ScienceControllerResearchcontract extends JController
{

	function display() {

		JRequest::setVar( 'view', 'researchcontract' );
		parent::display();
	}
	...
	function save_data() {
		...
		$model =& $this->getModel('researchcontract');
		$project_id = $model->store($post);

		if ($project_id) {
			$mainframe->enqueueMessage( JText::_('RESEARCHCONTRACT_STORE_OK') );
			if ($post['save']){
				JRequest::setVar('view', 'researchcontracts' );
			...
			$msg = JText::_('RESEARCHCONTRACT_STORE_KO');
			$this->setRedirect('index.php?option=com_science&view=researchcontracts', $msg);
		}
	}
}
