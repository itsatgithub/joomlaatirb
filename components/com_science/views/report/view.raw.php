			...
			switch ( $report )
			{
				...
				case 'collaborations':
					...
					if (count($items['collaborations']))
					{
						...
						for ($i=0, $n=count( $items ); $i < $n; $i++)
						{
							...
							$worksheet->write( $i+1, $j++, ($item->collaboration_id != $old_id) ? $item->id : '' );
				...
				case 'projects':
					...
					if (count($items['projects']))
					{
						...
						$worksheet->write(0, $j++, JText::_( 'PROJECT_AWARDING_DATE' ));
						...
						$worksheet->write(0, $j++, JText::_( 'PROJECT_TGB' ));
						...
						for ($i=0, $n=count( $items ); $i < $n; $i++)
						{
							...
							if (!$sci_user->isGroupLeader($user->username))
							{
								...
								if ($k <= $year_max)
								{
									$worksheet->write($i+1, $j++, $item->budget_year_6);
									$worksheet->write($i+1, $j++, $item->overheads_year_6);
									$k++;
								}
				...
				case 'researchcontracts':
					$model =& JModel::getInstance( 'researchcontracts', 'sciencemodel' );
					$items['researchcontracts'] = $model->getReportData( $start_date, $end_date, $groups );

					$j = 0; // colum index
					if (count($items['researchcontracts']))
					{
						$worksheet =& $workbook->addWorksheet( JText::_( 'RESEARCHCONTRACT_HEADER' ) );
						$worksheet->write(0, $j++, JText::_( 'ID' ));
						$worksheet->write(0, $j++, JText::_( 'RESEARCHCONTRACT_IRB_CODE' ));
						$worksheet->write(0, $j++, JText::_( 'RESEARCHCONTRACT_ACRONYM' ));
						$worksheet->write(0, $j++, JText::_( 'RESEARCHCONTRACT_GROUP_LEADER' ));
						$worksheet->write(0, $j++, JText::_( 'RESEARCHCONTRACT_COMPANY' ));
						...
						for ($i=0, $n=count( $items ); $i < $n; $i++)
						{
							...
							$worksheet->write($i+1, $j++, utf8_decode( $item->id ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->irb_code ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->acronym ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->gl_name ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->company ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->principal_investigator ));
							$worksheet->write($i+1, $j++, $item->start_date);
							$worksheet->write($i+1, $j++, $item->end_date);
				...
				case 'theses':
					...
					if (count($items['theses']))
					{
						...
						$worksheet->write(0, $j++, JText::_( 'ID' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_RESEARCH_PROGRAMME' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_GROUP_LEADER' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_TITLE' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_AUTHOR' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_UNIVERSITY' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_DOCTORAL_PROGRAMME' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_YEAR' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_LECTURE_DATE' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_LANGUAGE' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_DIRECTOR' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_CODIRECTOR' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_TUTOR' ));
						$worksheet->write(0, $j++, JText::_( 'THESIS_HONOR' ));
						...
						for ($i=0, $n=count( $items ); $i < $n; $i++)
						{
							...
							$worksheet->write($i+1, $j++, utf8_decode( $item->gl_name ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->title ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->author ));
							$worksheet->write($i+1, $j++, $item->university);
							$worksheet->write($i+1, $j++, utf8_decode ( $item->doctoral_programme ));
							$worksheet->write($i+1, $j++, $item->year);
							$worksheet->write($i+1, $j++, $item->lecture_date);
							$worksheet->write($i+1, $j++, $item->tl_description);
							$worksheet->write($i+1, $j++, utf8_decode( $item->director ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->codirector ));
							$worksheet->write($i+1, $j++, $item->tutor);
							$worksheet->write($i+1, $j++, $item->th_description);
						}
					}
					break;
				...
				case 'patents':
					...
					if (count($items['patents']))
					{
						...
						$worksheet->write(0, $j++, JText::_( 'ID' ));
						$worksheet->write(0, $j++, JText::_( 'PATENT_RESEARCH_PROGRAMME' ));
						$worksheet->write(0, $j++, JText::_( 'PATENT_GROUP_LEADER' ));
						$worksheet->write(0, $j++, JText::_( 'PATENT_TITLE' ));
						$worksheet->write(0, $j++, JText::_( 'PATENT_INVENTORS' ));
						$worksheet->write(0, $j++, JText::_( 'PATENT_PUBLICATION_NUMBER' ));
						$worksheet->write(0, $j++, JText::_( 'PATENT_PUBLICATION_DATE' ));
						$worksheet->write(0, $j++, JText::_( 'PATENT_STATUS' ));
						...
						for ($i=0, $n=count( $items ); $i < $n; $i++)
						{
							...
							$worksheet->write($i+1, $j++, utf8_decode( $item->gl_name ));
							$worksheet->write($i+1, $j++, $item->title);
							$worksheet->write($i+1, $j++, utf8_decode( $item->inventors ));
							$worksheet->write($i+1, $j++, $item->publication_number);
							$worksheet->write($i+1, $j++, $item->publication_date);
							$worksheet->write($i+1, $j++, $item->ps_description);
						}
					}
					break;
				...
				case 'awards':
					...
					if (count($items['awards']))
					{
						...
						$worksheet->write(0, $j++, JText::_( 'ID' ));
						$worksheet->write(0, $j++, JText::_( 'AWARD_RESEARCH_PROGRAMME' ));
						$worksheet->write(0, $j++, JText::_( 'AWARD_GROUP_LEADER' ));
						$worksheet->write(0, $j++, JText::_( 'AWARD_TITLE' ));
						$worksheet->write(0, $j++, JText::_( 'AWARD_BODY' ));
						$worksheet->write(0, $j++, JText::_( 'AWARD_AWARDEE' ));
						$worksheet->write(0, $j++, JText::_( 'AWARD_YEAR' ));
						$worksheet->write(0, $j++, JText::_( 'AWARD_END_YEAR' ));
						...
						for ($i=0, $n=count( $items ); $i < $n; $i++)
						{
							...
							$worksheet->write($i+1, $j++, utf8_decode( $item->gl_name ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->title ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->awarding_body ));
							$worksheet->write($i+1, $j++, utf8_decode( $item->awardee ));
							$worksheet->write($i+1, $j++, $item->year);
							$worksheet->write($i+1, $j++, $item->end_year);
						}
					}
					break;
			}
